...
public class CustomSpriteAnimator : MonoBehaviour
{
  ...
  {
    if (gameObject == null && currentAnimation != null) return;
    currentFrame++;
    if (currentAnimation != null && currentAnimation.triggers != null)
    {
      foreach (AnimationTrigger animationTrigger in currentAnimation.triggers)
      {
        if (animationTrigger.frame == currentFrame)
        {
          ...
          if (animationTrigger.name == "DestroyGameObject" || animationTrigger.name == "Destroy")
          {
            StopAllCoroutines();
            Destroy(gameObject);
          }

          if (animationTrigger.name == "Hide")
          {
            StopAllCoroutines();
            gameObject.SetActive(false);
          }

          if (animationTrigger.name.StartsWith("play,"))
          {
            string animationName = animationTrigger.name.Split(',')[1];
            currentFrame = 0;
            Play(animationName);
            return;
          }
